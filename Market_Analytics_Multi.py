...
# =========================================================
# EMAIL DEAL SUMMARY — TAB 2 BOTTOM
# =========================================================
st.markdown("---")
st.markdown("### 📨 Email Deal Summary to Investor")

recipient_email = st.text_input("Recipient Email", placeholder="sid@example.com", key="email_recipient")
sender_note = st.text_area("Optional Note", placeholder="Hi Sid, here is the analysis we discussed...", key="email_note", height=80)

if st.button("📤 Send Deal Summary", key="send_email_btn"):
    import re as _re
    if not recipient_email or not _re.match(r"[^@]+@[^@]+\.[^@]+", recipient_email):
        ...
        try:
            ...
            if not EMAIL_USER or not EMAIL_PASSWORD:
                ...
                yr1_cf_annual = results_a.get("annual_cash_flows", [-28092])[0] if results_a.get("annual_cash_flows") else -28092
                yr1_cf_monthly = yr1_cf_annual / 12
                deal_body = (
                    f"{sender_note if sender_note else 'Please find the deal summary below.'}\n\n"
                    f"{'='*42}\n"
                    f"MULTIFAMILY DEAL ANALYSIS SUMMARY\n"
                    f"Generated by RealEstate-Analytics.ai\n"
                    f"{'='*42}\n\n"
                    f"Property:         {address_a if address_a else '400 Main St, Suisun City CA 94585'}\n"
                    f"Units:            {int(num_units_a)} units\n"
                    f"Market ZIP:       {zip_input} -- {signal_text}\n\n"
                    f"-- ACQUISITION --\n"
                    f"Purchase Price:   ${acq_price_a:,.0f}\n"
                    f"Down Payment:     {down_pct_a:.0f}% (${acq_price_a * down_pct_a/100:,.0f})\n"
                    f"Loan Amount:      ${acq_price_a * (1 - down_pct_a/100):,.0f} @ {int_rate_a:.1f}% / {int(loan_term_a)} yr\n"
                    f"Monthly Mortgage: ${results_a.get('monthly_mortgage', 28941):,.0f}\n\n"
                    f"-- INCOME & EXPENSES --\n"
                    f"Gross Monthly Rent: ${gross_rent_a:,.0f}\n"
                    f"Vacancy ({vacancy_a:.0f}%):      -${gross_rent_a * vacancy_a/100:,.0f}\n"
                    f"Monthly Expenses:   -${expenses_a:,.0f}\n"
                    f"Net NOI/mo:         ${gross_rent_a * (1-vacancy_a/100) - expenses_a:,.0f}\n\n"
                    f"-- RETURNS --\n"
                    f"Cap Rate:         {results_a.get('cap_rate', 5.5):.2f}%\n"
                    f"Cash-on-Cash:     {results_a.get('cash_on_cash', -1.94):.2f}%\n"
                    f"IRR (Total):      {results_a.get('irr_total', 0):.2f}%\n"
                    f"Price / Unit:     ${acq_price_a / num_units_a:,.0f}\n"
                    f"Rent / Unit/Mo:   ${gross_rent_a / num_units_a:,.0f}\n\n"
                    f"-- CASH FLOW PROJECTION (3% rent growth) --\n"
                    f"Year 1 CF/mo:    ${yr1_cf_monthly:,.0f}\n"
                    f"Year 1 CF/yr:    ${yr1_cf_annual:,.0f}\n"
                    f"Breakeven Year:  ~Year 4\n\n"
                    f"-- MARKET CONTEXT --\n"
                    f"ZIP {zip_input}: {signal_text}\n"
                    f"Est. Cap Rate Range: {cap_low:.1f}% - {cap_high:.1f}%\n"
                    f"10-Yr Treasury: {treasury_rate:.2f}%\n\n"
                    f"-- NOTE --\n"
                    f"Expenses estimated at ${expenses_a:,.0f}/mo including property tax.\n"
                    f"Property tax alone on ${acq_price_a:,.0f} in CA ~ ${acq_price_a*0.011/12:,.0f}/mo.\n\n"
                    f"{'='*42}\n"
                    f"Masoud Arouni | RealEstate-Analytics.ai\n"
                    f"{'='*42}\n"
                )
                msg = EmailMessage()
                msg["Subject"] = f"Deal Analysis -- {address_a if address_a else '400 Main St Suisun City'} -- {int(num_units_a)} Units"
                msg["From"] = EMAIL_USER
                msg["To"] = recipient_email
                msg.set_content(deal_body)
                with smtplib.SMTP("smtp.gmail.com", 587) as smtp:
                    smtp.starttls()
                    smtp.login(EMAIL_USER, EMAIL_PASSWORD)
                    smtp.send_message(msg)
                st.success(f"✅ Deal summary sent to {recipient_email}!")
